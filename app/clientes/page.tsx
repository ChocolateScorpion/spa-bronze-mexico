        ...
        <table className="w-full min-w-[640px] text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {clientas.map((clienta) => {
              const digits = clienta.telefono.replace(/\D/g, "");
              const mensaje = encodeURIComponent(
                `Hola ${clienta.nombre}, te contactamos de Spa & Bronze México`
              );
              const waUrl = `https://wa.me/52${digits}?text=${mensaje}`;
              return (
                <tr key={clienta.id}>
                  ...
                  <td className="px-5 py-3 text-gray-500">
                    <span className="flex items-center gap-2">
                      ...
                      <a
                        ...
                        title={`Enviar WhatsApp a ${clienta.nombre}`}
                      >
